...
function prepareShipment($IDList, $profileID, $setting)
{
	global $gsetting;
	global $siteID, $sitePosition, $siteServer, $siteName, $siteEmail;
	global $arrEncodeStr;
	global $cuID;
	...
	//Lock tables
	query('Lock tables `' . $sitePosition . '.Order` O read, `' . $sitePosition . '.ShipmentV` S read, `' . $sitePosition . '.TransactionV` read', false);

	//Print the shipment
	$rs = query('Select S.*, O.`UserID` from `' . $sitePosition . '.ShipmentV` S inner join `' . $sitePosition . '.Order` O on S.`OrderID` = O.`ID` where O.`UserID` = ' . $cuID . ' and S.`ID` in (' . $IDList . ')', true);
	while(true)
	{
		$rsInfo = fetchRow($rs);
		...
		$stArr = array(
			'%SITEHOST%' => strtr($siteServer, $arrEncodeStr),
			...
			'%IID%' => $rsInfo[2],
			'%HID%' => $rsInfo[3],
			'%REFERENCEID%' => $rsInfo[10],
			'%TYPE%' => $rsInfo[11],
			'%UID%' => $rsInfo[12],
			'%ADDRESS%' => strtr(encodeAddress($rsInfo[6]), $arrEncodeStr),
			'%EXTRA_ADDRESS%' => strtr(encodeAddress($rsInfo[7] == '' ? $rsInfo[6] : $rsInfo[7]), $arrEncodeStr),
			'%YEAR%' => $dateInfo['year'],
		...
		$codeStr = array();
		$codeStr[] = $rsInfo[0] . '$' . $rsInfo[1] . '$' . $rsInfo[3] . '$' . $rsInfo[2] . '$' . $rsInfo[12] . '$' . $rsInfo[8] . '$' . $rsInfo[6] . '$' . $rsInfo[7];
