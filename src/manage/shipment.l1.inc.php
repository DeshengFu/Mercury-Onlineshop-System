...
function prepareShipment($IDList, $profileID)
{
	...
	//Print the shipment
	$rs = query('Select S.*, O.`UserID` from `' . $sitePosition . '.ShipmentV` S inner join `' . $sitePosition . '.Order` O on S.`OrderID` = O.`ID` where S.`ShopID` = ' . $shopID . ' and S.`ID` in (' . $IDList . ')', true);
	while(true)
	{
		$rsInfo = fetchRow($rs);
